Here is the complete Markdown file for Lesson 1, written strictly according to the Stratum Blueprint's pedagogical rules, section numbering requirements, and your newly authorized MyST admonition syntax.

You can copy the code block below and save it directly as `L01.md`.
...
- None — but read `BEFORE_YOU_BEGIN.md`
...
While important, technology choices are not architecture. **System architecture** is the high-level structure of a software system—the components it is made of, the specific responsibilities of each component, and the rules governing how those components interact. Architecture is about boundaries and communication.
...
Architecture does not happen in a vacuum. It is a specific phase within the **Software Development Lifecycle (SDLC)**. The SDLC describes the phases a project moves through: Requirements → Design (Architecture) → Implementation (Coding) → Testing → Deployment → Maintenance.

Skipping the design phase inevitably leads to **technical debt**. Note that technical debt is not the same as a bug. A bug is code that does not work. Technical debt is code that _does_ work, but was written in a way that makes future changes expensive and slow.
...
A designed system applies the principle of **separation of concerns**. We break the God class apart so that each component has one, clearly defined responsibility.
...
:::{tip}
Architecture is ultimately a communication tool. A well-designed system like the one in §4.2 communicates its intent clearly to the next developer who reads it.
:::
...
3.  Look at the "God class" diagram in §4.1. If the library decides to switch from sending overdue emails to sending overdue SMS text messages, why does the `Book` class need to be modified? Why is this architecturally problematic? `[BT3 — Apply]`
...
Now that you understand the difference between a system that grew and a system that was designed, we need to formalize how to evaluate those designs. In Lesson 2, we will dive deeper into "Separation of Concerns" and introduce the metrics of _Coupling_ and _Cohesion_ to measure the health of our architecture.
